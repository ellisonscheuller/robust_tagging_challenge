...
class Degradation(nn.Module):
    ...
    def __init__(
        self,
        # Severity argument here is the maximum severity
        # During training the severity is sampled from a uniform distribution from [0, max_severity]
        # We expect a severity on a scale of 0 to 1 (no degradation to maximal degradation)
        severity: float = 0.0
    ):
        super().__init__()
        self.n_eta_bins = 10
        self.n_phi_bins = 10
        self.max_num_miss = severity * (self.n_eta_bins * self.n_phi_bins)
